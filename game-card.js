...
(function () {
  ...
  // Fond orange marque (#FF8200 assombri en dégradé) pour tous : l'accent porte le palier.
  const TIER_STYLES = {
    bronze: { accent: "#c98a4b", soft: "#e8bd8f", bgTop: "#b35900", bgBottom: "#2e1600" },
    argent: { accent: "#aebcd0", soft: "#e2eaf5", bgTop: "#b35900", bgBottom: "#2e1600" },
    or: { accent: "#e8c34a", soft: "#f0d38c", bgTop: "#b35900", bgBottom: "#2e1600" },
    legende: { accent: "#7fd0ff", soft: "#cdeeff", bgTop: "#b35900", bgBottom: "#2e1600" },
    icone: { accent: "#c9a2ff", soft: "#ecdcff", bgTop: "#b35900", bgBottom: "#2e1600" },
  };
  ...
  function downloadCardImage() {
    const G = OE.G(); // carrière courante (remplacée à chaque partie)
    const filename = `road-to-the-glory-${G.name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}.png`;
    ...
    if (nativeOk) {
      (async () => {
        try {
          ...
          const file = new File([blob], "road-to-the-glory.png", { type: "image/png" });
